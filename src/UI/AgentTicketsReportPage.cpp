...
// void AgentTicketsReportPage::updateChart(const std::string& start, const std::string& end)
// {
//     // Xóa dữ liệu cũ
...
//     // Lấy báo cáo theo tháng cho agent hiện tại
//     auto* reports = reportManager_->generateMonthlyTicketReportForAgent(start, end);
...
//         categories << QString::fromStdString(r->month);
...
void AgentTicketsReportPage::updateChart()
{
    ...
    // Lấy dữ liệu vé theo tháng từ booking thực tế
    std::vector<int> monthlyTickets(12, 0);
    ...
    int maxTickets = 0;
    for (int i = 0; i < 12; ++i) {
        *barSet << monthlyTickets[i];
        if (monthlyTickets[i] > maxTickets) {
            maxTickets = monthlyTickets[i];
        }
    }

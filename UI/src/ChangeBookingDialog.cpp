...
void ChangeBookingDialog::setupUi()
{
    ...
    cancelBtn_ = new QPushButton("Hủy đổi vé", this);
...
void ChangeBookingDialog::showStep(int step)
{
    ...
    step1NextBtn_->setVisible(step < 2);
    confirmBtn_->setVisible(step == 2);
...
void ChangeBookingDialog::updatePriceDifference()
{
    ...
    if (diff > 0) {
        ...
        priceDiffLabel_->setStyleSheet("color:#D32F2F; font-weight:700; font-size: 14px;");
    }
    else if (diff < 0) {
        priceDiffLabel_->setText("Hoàn lại: " + dotFormat(-diff) + " VNĐ");
        priceDiffLabel_->setStyleSheet("color:#43A047; font-weight:700; font-size: 14px;");
    }
    else {
        priceDiffLabel_->setText("Không thay đổi giá");
        priceDiffLabel_->setStyleSheet("color:#1565C0; font-weight:700; font-size: 14px;");
    }
}
